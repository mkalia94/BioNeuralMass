...
function ampa(nm,FR_E,FR_ext)
    FR = FR_E + FR_ext;
    a_ampa = nm.a_ampa*(FR)/(FR + nm.ampa_th);
end

#GABA
function gaba(nm,FR_I,FR_ext)
    FR = FR_I + FR_ext;
    a_gaba = nm.a_gaba*(FR)/(FR + nm.gaba_th);
end
...
function Firing_Rate(nm,I,Ek,Ena)
    ...
    fun = x->((1/(nm.sigma*sqrt(2*pi)))*exp(-(I-x)^2/(nm.sigma)))*(kappa*sqrt(max(0,x-Ith)))*(1-(1+sign(x-Ith2))/2);
...
function (nm::BioNM)(x,p,t,expr=nothing)
    ...
    E_na_E                = 26.64*log(NaCe/NaCiE);          # [mV], Nernst potential sodium
    E_k_E                 = 26.64*log(KCe/KCiE);           # [mV], Nernst potential potassium
    E_cl_E                = 26.64*log(ClCiE/ClCe);          # [mV], Nernst potential chloride

    # Nernst potentials (Inhibitory population)
    E_na_I                = 26.64*log(NaCe/NaCiI);
    E_k_I                 = 26.64*log(KCe/KCiI);
    E_cl_I                = 26.64*log(ClCiI/ClCe);

    # Equilibrium potentials (variable)
    I_EE = off_*(nm.g_EE*rAMPA*(NaCe - NaCiE)) 
    I_IE =  off_*(nm.g_IE*rGABA*(ClCe - ClCiE))
    I_EI =off_*(nm.g_EI*rAMPA*(NaCe - NaCiI))
    I_II = off_*nm.g_II*rGABA*(ClCe - ClCiI)
   ...
   # Ion pump
    ...
    fpump_I = 1/(1+0.1245*exp(-0.1*nm.F/nm.R/nm.T*V_I) +
               0.0365*sigmapump*exp(-nm.F/nm.R/nm.T*V_I)) 
    fpump_E = 1/(1+0.1245*exp(-0.1*nm.F/nm.R/nm.T*V_E) +
               0.0365*sigmapump*exp(-nm.F/nm.R/nm.T*V_E)) 
    ...
    FR_E                  = Firing_Rate(nm,I_E-Ipump_E+IExcite,E_k_E,E_na_E);                                     # [1/ms], Firing Rate
    ...
    alpha_n     = 0.01 * (V_E+34.0)/( 1.0 - exp(-0.1 * (V_E+34.0)) ); #[no units]
    beta_n      = 0.125 * exp(-(V_E+44.0)/80.0);
    alpha_m     = 0.1 * (V_E+30.0)/( 1.0 - exp(-0.1 * (V_E+30.0)) );
    beta_m      = 4.0 * exp(-(V_E+55.0)/18.0);
    alpha_h     = 0.07 * exp(-(V_E+44.0)/20.0);
    beta_h      = 1.0/( 1.0 + exp(-0.1 * (V_E+14.0)) );
    ...
    I_GCl_E     = (nm.PclG/(1+exp(-(V_E+10)/10)))*(nm.F*nm.xi)*V_E*((ClCiE-ClCe*exp(nm.xi*V_E))/(1-exp(nm.xi*V_E)));    # Voltage gated chloride current

    # Voltage-gated sodium/potassium/chloride current (inhibitatory population)
    alpha_n     = 0.01 * (V_I+34.0)/( 1.0 - exp(-0.1 * (V_I+34.0)) ); #[no units]
    beta_n      = 0.125 * exp(-(V_I+44.0)/80.0);
    alpha_m     = 0.1 * (V_I+30.0)/( 1.0 - exp(-0.1 * (V_I+30.0)) );
    beta_m      = 4.0 * exp(-(V_I+55.0)/18.0);
    alpha_h     = 0.07 * exp(-(V_I+44.0)/20.0);
    beta_h      = 1.0/( 1.0 + exp(-0.1 * (V_I+14.0)) );
    m_inf       = alpha_m/(alpha_m + beta_m);
    n_inf       = alpha_n/(alpha_n + beta_n);
    h_inf       = alpha_h/(alpha_h + beta_h);
    ...
    I_GCl_I               = (nm.PclG/(1+exp(-(V_I+10)/10)))*(nm.F*nm.xi)*V_I*((ClCiI-ClCe*exp(nm.xi*V_I))/(1-exp(nm.xi*V_I)));
...
dX = [(1/(nm.F))*(-I_LNa_E-I_TNa_E - 3*Ipump_E); #  + 1/nm.F*I_EE;           # Na_i excitatory
 ...
 # dX = dX*60*1f3
    ...
    elseif expr == "I_EI"    ; return I_EI
    elseif expr == "I_EE"    ; return I_EE
    elseif expr == "I_IE"    ; return I_IE
    elseif expr == "I_II"     ; return I_II
end
end
